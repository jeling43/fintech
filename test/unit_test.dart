...
void main() {
  ...
  group('HashService', () {
    ...
    test('computeHash accepts raw bytes and returns 64-char hex', () {
      final bytes = [0x48, 0x65, 0x6c, 0x6c, 0x6f]; // "Hello"
      final hash = hashService.computeHash(bytes);
      expect(hash.length, 64);
    });

    test('computeHash produces consistent results for same bytes', () {
      final bytes = [1, 2, 3, 4, 5];
      final hash1 = hashService.computeHash(bytes);
      final hash2 = hashService.computeHash(bytes);
      expect(hash1, equals(hash2));
    });

    test('computeHash produces different results for different bytes', () {
      final hash1 = hashService.computeHash([1, 2, 3]);
      final hash2 = hashService.computeHash([4, 5, 6]);
      expect(hash1, isNot(equals(hash2)));
    });
  });
  ...
  // PdfImportInput
  // ---------------------------------------------------------------------------
  group('PdfImportInput', () {
    test('stores bytes and filename', () {
      final bytes = Uint8List.fromList([0x25, 0x50, 0x44, 0x46]); // %PDF
      const filename = 'statement.pdf';
      final input = PdfImportInput(bytes: bytes, filename: filename);
      expect(input.bytes, equals(bytes));
      expect(input.filename, equals(filename));
    });

    test('bytes and filename are independent for different instances', () {
      final bytes1 = Uint8List.fromList([1, 2, 3]);
      final bytes2 = Uint8List.fromList([4, 5, 6]);
      final a = PdfImportInput(bytes: bytes1, filename: 'a.pdf');
      final b = PdfImportInput(bytes: bytes2, filename: 'b.pdf');
